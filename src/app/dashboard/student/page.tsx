...
export default function StudentDashboard() {
  ...
  const sidebarItems: { key: STab; icon: any; label: string }[] = [
    { key: "home", icon: IoGrid, label: lang === "ar" ? "الرئيسية" : "Home" },
    ...
    { key: "homework", icon: IoDocuments, label: lang === "ar" ? "الواجبات" : "Homework" },
    { key: "files", icon: FaFileAlt, label: lang === "ar" ? "الملفات" : "Files" },
    { key: "reports", icon: IoStatsChart, label: lang === "ar" ? "التقارير" : "Reports" },
    { key: "achievements", icon: IoTrophy, label: lang === "ar" ? "الإنجازات" : "Achievements" },
    { key: "profile", icon: IoPerson, label: lang === "ar" ? "الملف الشخصي" : "Profile" },
    { key: "parent", icon: IoPeople, label: lang === "ar" ? "ولي الأمر" : "Parent" },
    { key: "ai", icon: FaRobot, label: lang === "ar" ? "المساعد الذكي" : "AI Assistant" },
    { key: "game", icon: FaGamepad, label: lang === "ar" ? "لعبة اليوم" : "Daily Game" },
  ];
  ...
  const renderTab = () => {
    switch (tab) {
      ...
      case "exams": return activeExam ? <ExamPlayer /> : <ExamsTab />;
  ...
  const HomeTab = () => {
    ...
    const avgScore = examResults.length > 0 ? Math.round(examResults.reduce((a, r) => a + (r.score / r.total) * 100, 0) / examResults.length) : 0;
    const stats = [
      { icon: IoBook, bg: "bg-primary-light", color: "text-primary", value: lessons.filter(l => l.viewers?.[user!.uid]).length.toString(), label: lang === "ar" ? "الدروس المكتملة" : "Completed Lessons" },
            ...
            {lessons.map(l => (
              <div key={l.id} className="bg-white rounded-[20px] overflow-hidden shadow-sm border border-border transition-all duration-300 hover:-translate-y-0.5 hover:shadow-md">
                ...
                <div className="p-4">
                  ...
                  <p className="text-xs text-text-light mt-1">{lang === "ar" ? `المشاهدات: ${l.viewers?.[user!.uid] || 0}` : `Views: ${l.viewers?.[user!.uid] || 0}`}</p>
                  <button onClick={async () => { await incrementLessonView(l.id, user!.uid); setPlaying(l); }} className="mt-3 w-full px-3 py-2 rounded-full text-xs font-semibold bg-gradient-to-r from-primary to-accent text-white cursor-pointer border-none">{lang === "ar" ? "مشاهدة" : "Watch"}</button>
                </div>
              </div>
            ))}
  ...
  const ExamPlayer = () => {
    ...
    const submitExam = () => {
      ...
      const total = activeExam.questions.reduce((a, q) => a + (q.points || 1), 0);
      const passed = score / total >= (activeExam.passScore || 50) / 100;
      setExamScore(score);
      setExamSubmitted(true);
      submitExamResult({ examId: activeExam.id, studentId: user!.uid, score, total, passed, answers, submittedAt: new Date() as any });
    };
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {homework.map(h => (
          <div key={h.id} className="bg-white rounded-[20px] p-5 shadow-sm border border-border">
            ...
            <button onClick={() => setSelectedHomework(selectedHomework === h.id ? null : h.id)} className="mt-3 px-4 py-2 rounded-full text-xs font-semibold bg-gradient-to-r from-primary to-accent text-white cursor-pointer border-none">
              {lang === "ar" ? "رفع الحل" : "Submit Solution"}
            </button>
          </div>
        ))}
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {lockedFiles.map(f => {
            ...
            const canAccess = lesson?.viewers?.[user!.uid];
            return canAccess ? (
              <a key={f.id} href={f.url} target="_blank" className="bg-white rounded-[20px] p-4 shadow-sm border border-border flex items-center gap-3 hover:-translate-y-0.5 transition-all">
                <FaFileAlt className="text-primary text-xl" />
                <span className="text-sm font-medium">{f.name}</span>
  ...
  const AITab = () => {
    const sendAiMessage = async () => {
      ...
      setTimeout(() => {
        ...
        setAiChat((prev: any[]) => [...prev, { role: "ai", text: reply }]);
      }, 500);
    };
    return (
      <div className="max-w-2xl">
  ...
  const GameTab = () => {
    ...
    const endGame = async () => {
      setGameActive(false);
      await saveGameScore({ studentId: user!.uid, gameType: gameType!, score: gameScore });
      ...
      <div>
        ...
        {gameActive && (
          <div className="mt-6 bg-white rounded-[20px] p-6 shadow-sm border border-border text-center">
            ...
            <button onClick={() => setGameScore((prev: number) => prev + 10)} className="px-6 py-3 rounded-full text-sm font-semibold bg-gradient-to-r from-primary to-accent text-white mr-3 cursor-pointer border-none">+10</button>
  ...
  // ====== COMMUNITY (always visible in sidebar footer) ======
  ...
  const CommunityModal = () => {
    ...
    const send = async () => {
      ...
      await sendMessage({ channelId, senderId: user!.uid, senderName: userName || "User", text: chatText });
      setChatText("");
      const msgs = await fetchMessages(channelId);
      setChatMessages(msgs);
    };
          ...
          <div className="flex-1 p-4 overflow-y-auto space-y-3 h-[300px]">
            {chatMessages.map(msg => (
              <div key={msg.id} className={`flex ${msg.senderId === user!.uid ? "justify-end" : "justify-start"}`}>
                <div className={`max-w-[80%] p-3 rounded-xl text-sm ${msg.senderId === user!.uid ? "bg-primary text-white" : "bg-bg text-text"}`}>
                  <p className="text-xs font-semibold mb-1 opacity-70">{msg.senderId === user!.uid ? lang === "ar" ? "أنت" : "You" : msg.senderName}</p>
                  {msg.text}
                </div>
              </div>
            ))}
  ...
  useEffect(() => {
    if (showLeaderboard) fetchLeaderboard().then(setLeaderboard);
  }, [showLeaderboard]);
